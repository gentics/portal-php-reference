	...
	<div class="starReviews col-xl-4 col-lg-3 col-sm-8 row" :class="{ 'd-none': !visible }" v-on-clickaway="close">
		<div class="review-left col-lg-6 col-sm-12">
			<div v-for="rate in ratesReversed" class="progress">
				<div class="progress-bar text-left pl-2"
					 role="progressbar"
					 :style="'width: ' + getRatePercentage(rate) + '%'"
					 :aria-valuenow="getRatePercentage(rate)"
					 aria-valuemin="0"
					 aria-valuemax="100"
					 :title="getRatePercentage(rate) + '%'">
					{{ rate.rate }} {{ rate.rate  | pluralize('en', ['star', 'stars']) }}
				</div>
			</div>
		</div>
		<div class="review-right col-lg-6 col-sm-12" :class="{ 'rate-saving': updatingRate }">
			<p class="mb-0 mt-3">
				<template v-if="currentRate !== null">
					You rated this for {{ currentRate }} {{ currentRate | pluralize('en', ['star', 'stars']) }}
				</template>
				<template v-else>
					Leave your rating
				</template>
			...
			<div class="rating-stars text-center pt-3">
				<a v-for="rate in ratesReversed" :class="{ 'active': isStarActive(rate) }" @click="setRate(rate)">
